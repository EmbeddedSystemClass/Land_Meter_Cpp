...
DCMotor::DCMotor(int number) {
	...
	cout << "DCMotor "<< number << " set to path: " << path << endl;
}

int DCMotor::write(string path, string filename, string value){
   ofstream fs;
   cout << path << filename << value << endl;
   fs.open((path + filename).c_str());
   if (!fs.is_open()){
	   perror("DCMotor: write failed to open file ");
	   cout << path << filename << " " << value << endl;
	   return -1;
   }
   fs << value;
...
string DCMotor::read(string path, string filename){
   ifstream fs;
   fs.open((path + filename).c_str());
   if (!fs.is_open()){
	   perror("DCMotor: read failed to open file ");
    }
   string input;
   getline(fs,input);
   fs.close();
...
int DCMotor::write(string path, string filename, int value){
   cout << path << filename << value << endl;
   stringstream s;
   s << value;
...
int DCMotor::setValue(GPIO_VALUE value){
...
int DCMotor::setEdgeType(GPIO_EDGE value){
   switch(value){
...
int DCMotor::streamOpen(){
	stream.open((path + "value").c_str());
	return 0;
}
int DCMotor::streamWrite(GPIO_VALUE value){
	stream << value << std::flush;
	return 0;
}
int DCMotor::streamClose(){
	stream.close();
	return 0;
}

int DCMotor::toggleOutput(){
...
int DCMotor::toggleOutput(int time){ return this->toggleOutput(-1, time); }
int DCMotor::toggleOutput(int numberOfTimes, int time){
	this->setDirection(OUTPUT);
    ...
    if(pthread_create(&this->thread, NULL, &threadedToggle, static_cast<void*>(this))){
    	perror("DCMotor: Failed to create the toggle thread");
    	this->threadRunning = false;
    	return -1;
    }
    return 0;
...
int DCMotor::waitForEdge(){
    ...
    if (epoll_ctl(epollfd, EPOLL_CTL_ADD, fd, &ev) == -1) {
       perror("DCMotor: Failed to add control interface");
       return -1;
    }
	while(count<=1){  // ignore the first trigger
		i = epoll_wait(epollfd, &ev, 1, -1);
		if (i==-1){
			perror("DCMotor: Poll Wait fail");
...
int DCMotor::waitForEdge(CallbackType callback){
	this->threadRunning = true;
	this->callbackFunction = callback;
    // create the thread, pass the reference, address of the function and data
    if(pthread_create(&this->thread, NULL, &threadedPoll, static_cast<void*>(this))){
    	perror("DCMotor: Failed to create the poll thread");
    	this->threadRunning = false;
    	return -1;
    }
    return 0;
